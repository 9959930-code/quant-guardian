...
BTC 기능은 **장기 반감기 연구 v0.4 모드**입니다. 업비트 `KRW-BTC` 확정 일봉, Yahoo `BTC-USD`·USD/KRW, mempool.space·Blockstream 블록 높이, Coin Metrics Community 무료 온체인 이력으로 반감기·가치·40주 추세 전략을 계산합니다. 자동주문은 없고 기존 ETF 웹 화면과 텔레그램에는 아직 연결하지 않았습니다.
...
2016년부터 합성 원화 가격으로 반감기·40주 추세 v0.4를 재현하려면 다음을 실행합니다.

```powershell
python btc_cycle_research.py --refresh
```

2026-08-10 스냅샷에서 기존 v0.3 고정 기준안을 2016년까지 그대로 연장하면 1,000만 원은 1,316,713,556원(CAGR 58.40%, MDD -67.56%)으로 계산됐습니다. 새 v0.4 원안은 1,022,426,167원(CAGR 54.67%, MDD -61.08%), 후기과열 40% 잠금 진단안은 981,022,652원(CAGR 54.06%, MDD -56.94%)입니다. Upbit 상장 전 합성가격, 세금 제외, 과거 결과를 본 뒤 설계한 규칙이라는 한계가 있으므로 미래 기대수익이나 실전 승인값이 아닙니다. 상세 내용은 `docs/btc/v0.4/`에 있습니다.
