...
BTC 기능의 최신 후보는 **가격 모멘텀·변동성 연구 v0.5**입니다. 1·3·6·12개월 수익률의 양수 개수로 BTC 기본비중을 정하고 63일 변동성으로 비중 상한을 계산합니다. 반감기·MVRV·RSI는 v0.5 매매식에 사용하지 않습니다. 자동주문은 없고 기존 ETF 웹 화면과 텔레그램에는 아직 연결하지 않았습니다.
...
v0.5 모멘텀·변동성 연구를 재현하려면 다음을 실행합니다.

```powershell
python btc_momentum_research.py --refresh
```

같은 2016~2026 스냅샷에서 1,000만 원은 953,180,839원(누적수익 9,431.81%, CAGR 53.65%, MDD -43.73%)으로 계산됐습니다. 실제 Upbit 가격을 사용할 수 있는 2018-09 이후 교차검증에서는 143,017,254원(CAGR 40.18%, MDD -44.31%)입니다. 이 결과도 과거 연구값이며 상세 규칙과 한계는 `docs/btc/v0.5/`에 있습니다.
...
- `btc_cycle_research.py`: 2016년 합성 원화와 반감기·40주 추세 v0.4 연구
- `btc_momentum_research.py`: 1·3·6·12개월 모멘텀과 변동성 예산 v0.5 연구
