...
GitHub Actions 예약 실행 사이가 90분 이상 벌어지면 다음 정상 실행에서 `자동화 지연 감지 · 복구` 메시지를 보낸다. 이 알림은 중단 중에 즉시 전송하는 외부 모니터가 아니라, 실행이 재개된 뒤 공백을 알려주는 복구형 감지다. 반복 알림 폭주를 막기 위해 12시간 냉각시간을 둔다.
